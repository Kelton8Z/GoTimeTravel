...
from . import board as kataboard
from . import modelconfigs
...
class Features:
    ...
    def loc_to_tensor_pos(self,loc,board):
        if loc == kataboard.Board.PASS_LOC:
    ...
    def iterLadders(self, board, f):
        ...
        for y in range(bsize):
            for x in range(bsize):
                ...
                if stone == kataboard.Board.BLACK or stone == kataboard.Board.WHITE:
    ...
    def fill_row_features(self, board, pla, opp, boards, moves, move_idx, rules, bin_input_data, global_input_data, idx):
        ...
        if move_idx >= 1 and moves[move_idx-1][0] == opp:
            ...
            if prev1_loc is not None and prev1_loc != kataboard.Board.PASS_LOC:
                pos = self.loc_to_tensor_pos(prev1_loc,board)
                bin_input_data[idx,pos,9] = 1.0
            elif prev1_loc == kataboard.Board.PASS_LOC:
        ...
        if rules["scoringRule"] == "SCORING_TERRITORY":
            selfKomi = (whiteKomi+1 if pla == kataboard.Board.WHITE else -whiteKomi)
        else:
            selfKomi = (whiteKomi if pla == kataboard.Board.WHITE else -whiteKomi)
